...
class GadgetSearch(object):
    ...
    def get_format_count(self, gadget_format: str) -> int:
        ...
        # (took an example from stack overflow)
        unique_placeholders = set([fname for _, fname, _, _ in string.Formatter().parse(gadget_format) if fname])
    ...
    def get_register_combos(self, nregs: int, registers: Tuple[str]) -> Iterable[Iterable[str]]:
        ...
        # TODO: IMPLEMENT THIS FUNCTION
        all_combos = []
        for comb in product(registers, repeat=nregs):
    ...
    def format_all_gadgets(self, gadget_format: str, registers: Tuple[str]) -> Iterable[str]:
        ...
        register_combos = self.get_register_combos(num_placeholders, registers)
        all_gadgets = []
        for comb in list(register_combos):
            all_gadgets.append(gadget_format.format(*comb))
    ...
    def find_all(self, gadget: str) -> Iterable[int]:
        ...
        # TODO: IMPLEMENT THIS FUNCTION
        all_addresses = []
        assembly_wanted = assemble.assemble_data(gadget + '; RET')
        file_content = self.dump
        start_address = self.start_addr

        i = 0
        while i < len(file_content):
            # if found, res will be set to the index where the next gadget starts. otherwise, -1
            res = file_content.find(assembly_wanted, i)
            if res == -1:
                break
            all_addresses.append(start_address + res)
            # search again after the gadget
            i = res + len(assembly_wanted)
    ...
    def find_all_formats(self, gadget_format: str,
        ...
        all_formats_addresses = []
        for gadget in list(possible_gadgets):
            all_formats_addresses.append(self.find_all(gadget))
        return all_formats_addresses
